namespace MicroServicesStarter.ServiceManagement.Debugger
{
    using System;
    ...
    public class VisualStudioAttacher
    {
            ...
            if (TryGetVsInstance(visualStudioProcess.Id, out visualStudioInstance))
            {
                while (true)
                {
                    try
                    {
                        ...
                        EnvDTE.Process processToAttachTo =
                            visualStudioInstance.Debugger.LocalProcesses.Cast<EnvDTE.Process>()
        ...
        public static Process GetVisualStudioForSolution(string solutionDirectory)
        {
            IEnumerable<Process> visualStudios = GetVisualStudioProcesses();
            ...
            foreach (Process visualStudio in visualStudios)
            {
                ...
                if (TryGetVsInstance(visualStudio.Id, out visualStudioInstance))
                {
                    try
                    {
                        solutionDirectory = solutionDirectory.Trim(new[] { '\\' });

                        var actualSolutionDirectory = Path.GetDirectoryName(visualStudioInstance.Solution.FullName);

                        if (string.Compare(actualSolutionDirectory, solutionDirectory, StringComparison.InvariantCultureIgnoreCase) == 0)
                        {
                            return visualStudio;
                        }
                    }
